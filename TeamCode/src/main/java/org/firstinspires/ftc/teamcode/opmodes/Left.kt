...
open class Left : AutonomousTemplate() {
    ...
    override val startPose = Pose2d(negateIfReversed(-36.0), -65.5, Math.PI / 2)
    ...
    override fun initializeTrajectories() {
        ...
        intake = drive.trajectorySequenceBuilder(deliverPreload.end()) // Intake
                ...
                .splineToLinearHeading(Pose2d(negateIfReversed(-46.0), -12.0, if (reversed) Math.PI else 0.0), if (reversed) 0.0 else Math.PI)
                .setReversed(false)
                .back(17.5)
                .build()
        delivery = drive.trajectorySequenceBuilder(intake.end()) // Delivery
                .forward(22.5)
                .splineToLinearHeading(Pose2d(negateIfReversed(-28.6), -4.6, if (reversed) 3 * Math.PI / 4 else Math.PI / 4), Math.PI / 2)
                .build()
        park = drive.trajectorySequenceBuilder(delivery.end())
                .setReversed(true) // splines always start by moving forward, so we need to call backward forward
                .splineToLinearHeading(Pose2d(negateIfReversed(-36.0), -12.0, if (reversed) Math.PI else 0.0), -Math.PI / 2)
                .setReversed(false)
                .strafeRight(negateIfReversed(24.0))
                .build()
        if (parkPosition != 0) finalMovement = drive.trajectoryBuilder(deliverPreload.end())
                .forward(negateIfReversed(parkPosition.toDouble()))
                .build()
    }
    ...
    override fun main() {
        ...
        for (i in 0 until ADDITIONAL_CYCLES) {
            ...
            currentCycle = createCycle(LiftInternals.Position.HIGH,
                    LiftInternals.Position.fromStackHeight(max(4 - i, 1)))
            currentCycle.start()
        ...
        drive.followTrajectory(finalMovement) // TODO fix this
    }
