...
class Underworld : AutonomousTemplate() {
    ...
    override fun initializeTrajectories(): Pose2d {
        preload = drive.trajectorySequenceBuilder(Pose2d(-36.0, -63.0, PI))
            .strafeTo(Vector2d(-36.0, 0.0))
            .lineTo(Trajectories.claw2Bot(-24.0, 0.0, true, PI).into().vec())
            .build()
        toStack = drive.trajectorySequenceBuilder(preload.end())
            .strafeTo(Vector2d(-36.0, 0.0))
            .strafeTo(Vector2d(-36.0, -12.0))
            .lineTo(Trajectories.claw2Bot(-70.0, -12.0, false, PI).into().vec())
            .build()
